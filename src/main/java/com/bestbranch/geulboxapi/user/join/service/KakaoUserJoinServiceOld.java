...
public class KakaoUserJoinServiceOld implements UserJoinService {
    private final UserSpringDataJpaRepository userRepository;

    @Transactional
    @Override
    public UserView joinUser(UserJoinRequest param) {
        Optional<User> nullableUser = userRepository.findByThirdPartyAccountIdAndUserAuthenticationType(param.getThirdPartyAccountId(), User.AuthenticationType.KAKAO);
        User user;
        if (!nullableUser.isPresent()) {
            user = User.from(param);
        } else {
            user = nullableUser.get();
            user.setTokens(param.getAccessToken(), param.getRefreshToken());
